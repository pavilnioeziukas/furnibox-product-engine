...
def load_latest_validated_dataset(
    ...
    """Nuskaito naujausią Production Validated Product Dataset."""
    ...
    if not isinstance(record.get("products"), list):
        raise ValueError(
            "Validated Dataset neturi products sąrašo."
        )

    return record, path


def validated_synthetic_apacks(
    dataset_record: dict,
) -> dict[str, dict]:
    """Grąžina tik Dataset patvirtintas sintetines APACK korteles."""
    result = {}
    ...
    for product in dataset_record.get("products", []):
        ...
        if not generated_from.startswith("FPACK-"):
            continue

        if product_type != "PREPACK CABINETS":
            continue
...
def find_reference_export(base: Path) -> Path | None:
    """Grąžina naujausią etaloninį eksportą arba None.

    Etalonas nebėra privalomas, kai generuojamos tik sintetinės poros
    (APACK, CABINET-A, HRD-A). Taip Production trūkstamos APACK kortelės
    gali būti paruoštos neturint seno Stage Excel eksporto.
    """
    candidates = []
    for folder in (base / "data", base):
        candidates.extend(
            folder.glob(
                "Product (product.template)*.xlsx"
            )
        )
...
def build_rows(
    ...
    # APACK korteles generuojame tik pagal Validated Product Dataset.
    # Žalias Reform failas nėra autoritetas sintetinių APACK sąrašui.
    scheduled_skus = {
        canon(row["Internal reference"])
        for row in ready + review
    }

    for generated_code, dataset_product in sorted(
        validated_apacks.items()
    ):
        if (
            generated_code in existing_skus
            or generated_code in scheduled_skus
        ):
            continue
        ...
        source_product = reform_products.get(
            source_sku
        )

        if source_product is None:
            diagnostics.append(
                f"{generated_code}: Dataset šaltinis "
                f"{source_sku or 'NERASTAS'} nerastas žaliame Reform faile"
            )
            continue

        if not apack_category:
            diagnostics.append(
                f"{generated_code}: nenustatyta APACK kategorija"
            )
            continue

        # APACK profilis nepriklauso nuo FPACK Reform kategorijos.
        # Ankstesnė versija kvietė parent_row(), kuri reikalavo category_map.
        # Kai Production netrūko nė vieno raw Reform produkto, category_map
        # FPACK kategorijoms nebuvo sudaromas ir teisėtos APACK kortelės
        # būdavo klaidingai praleidžiamos.
        duplicate = {
            "Internal reference": generated_code,
            "name": (
                APPROVED_PRODUCT_NAMES.get(source_sku)
                or source_product.get("name_1")
                or source_product.get("name_2")
                or generated_code
            ),
            "route_ids/id": route_profile["apack"],
            "type": "Storable Product",
            "categ_id": apack_category["external_id"],
            "invoice_policy": "Delivered quantities",
            "packaging_name": packaging_name(source_sku),
            "variant_seller_ids/partner_id/id": "",
            "Reform Role": "GENERATED APACK",
            "Reform Category / Part Group": "APACK",
            "Review reason": "",
        }
...
def main():
    parser = argparse.ArgumentParser(description="Generuoja trūkstamų Reform ir sintetinių porų produktų importą.")
    parser.add_argument(
        "--env-file",
        type=Path,
    ...
    parser.add_argument(
        "--dataset",
        type=Path,
        help="Validated Product Dataset JSON. Jei nenurodyta, imamas naujausias Production.",
    )
    ...
    print(
        "Nuskaitomi visi Reform produktai "
        "ir komponentai..."
    )
    reform_products, _, _ = (
    ...
    missing_validated_apacks = sorted(
        set(validated_apacks)
        - set(odoo_products)
    )
    print("Trūksta pasirinktoje Odoo aplinkoje:", len(missing_skus))
    print(
        "Trūksta Dataset sintetinių APACK:",
        len(missing_validated_apacks),
    )
