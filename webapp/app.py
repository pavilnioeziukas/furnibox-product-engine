        ...
        for values in rows:
            ...
            if sku.casefold() == normalized:
                match = {
                    ...
                    "markup_percent": row.get("Final Markup Percent", 0),
                    "markup_amount": row.get("Final Markup Amount", 0),
    ...
    try:
        ...
        if raw:
            match = {
                ...
                "markup_percent": raw.get("Final Markup Percent", 0),
                "markup_amount": raw.get("Final Markup Amount", 0),
...
def _bootstrap_config(environment: str) -> OdooConfig:
    ...
    values = {
        ...
        "database": (os.getenv(prefix + "DATABASE") or os.getenv(prefix + "DB", "")).strip(),
        "username": (os.getenv(prefix + "USERNAME") or os.getenv(prefix + "LOGIN", "")).strip(),
        "api_key": os.getenv(prefix + "API_KEY", "").strip(),
    }
    missing = [name for name, value in values.items() if not value]
    if missing:
        raise ValueError(f"{BOOTSTRAP_ENVIRONMENTS[environment]} aplinkai trūksta nustatymų: " + ", ".join(missing))
...
def _write_bootstrap_audit(environment: str, operation: str, result: str) -> None:
    entry = {"created_at": utc_now(), "environment": environment, "operation": operation, "result": result, "actor": request.remote_addr or "unknown"}
...
def bootstraps():
    ...
    for key, label in BOOTSTRAP_ENVIRONMENTS.items():
        try:
            state = delivered_bootstrap_status(BootstrapClient(_bootstrap_config(key)))
        except Exception as exc:
            state = {"state": "error", "detail": str(exc)}
        environments.append({"key": key, "label": label, **state})
    return render_template("bootstraps.html", environments=environments, action_name=DELIVERED_ACTION_NAME, csrf_token=csrf_token, audit=_read_bootstrap_audit())
...
def change_delivered_quantity_bootstrap(environment: str, operation: str):
    ...
    if environment not in BOOTSTRAP_ENVIRONMENTS or operation not in {"install", "uninstall"}:
        abort(404)
    if not secrets.compare_digest(request.form.get("csrf_token", ""), session.get("bootstrap_csrf", "")):
    ...
    try:
        ...
        if operation == "install":
            ...
            message = (f"{BOOTSTRAP_ENVIRONMENTS[environment]}: bootstrap pašalintas." if result else f"{BOOTSTRAP_ENVIRONMENTS[environment]}: bootstrap nebuvo įdiegtas.")
