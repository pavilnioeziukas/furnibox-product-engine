...
MTO_ROUTE = "stock.route_warehouse0_mto"
MANUFACTURE_ROUTE = "mrp.route_warehouse0_manufacture"
APACK_ROUTE = "__export__.stock_route_18_32d69257"
...
def infer_category_map(client: OdooClient, metadata: dict[str, dict]):
    ...
    mapping = {}
    diagnostics = []
    for reform_category in sorted({meta["category"] for meta in metadata.values()}):
...
def routes_for(sku: str, bom_parents: set[str]) -> str:
    if sku.startswith("APACK-"):
        return f"{MTO_ROUTE},{APACK_ROUTE}"
    if sku in bom_parents:
        return f"{MTO_ROUTE},{MANUFACTURE_ROUTE}"
    return MTO_ROUTE


def build_rows(new_skus, metadata, bom_parents, category_map, apack_category):
    import_rows = []
    review_rows = []
    diagnostics = []
    for sku in sorted(new_skus):
        ...
        row = {
            ...
            "route_ids/id": routes_for(sku, bom_parents),
        ...
        if sku.startswith("FPACK-"):
            ...
            duplicate["route_ids/id"] = f"{MTO_ROUTE},{APACK_ROUTE}"
...
def write_workbook(path: Path, rows, extra_columns=False, category_rows=None, diagnostics=None, info=None):
    wb = Workbook()
    ws = wb.active
...
def main():
    ...
    category_map, apack_category, category_diagnostics = infer_category_map(client, metadata)
    import_rows, review_rows, row_diagnostics = build_rows(
        new_skus, metadata, bom_parents, category_map, apack_category
    )
    diagnostics = category_diagnostics + row_diagnostics
    ...
    write_workbook(review_path, review_rows, extra_columns=True,
                   category_rows=category_rows, diagnostics=diagnostics, info=info)
