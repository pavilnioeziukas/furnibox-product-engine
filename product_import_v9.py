...
def build_rows(
    ...
    for generated_code, dataset_product in sorted(
        ...
        if duplicate is None:
            diagnostics.append(
                f"{generated_code}: nenustatyta šaltinio "
                f"{source_sku} produkto kategorija"
            )
            continue
        ...
        duplicate["packaging_name"] = packaging_name(
            source_sku
        )
        duplicate["Reform Role"] = "GENERATED APACK"
        duplicate[
            "Reform Category / Part Group"
        ] = "APACK"
