...
class FurniboxApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.bom_button = ttk.Button(
            ...
            command=lambda: self.run_script(first_existing("bom_type_inference_v3.py", "bom_type_inference_v2.py", "bom_type_inference_v1.py")),
    ...
    def open_output(self):
        ...
        if selected is None:
            return
        old_name, old_url = os.environ.get("FURNIBOX_ENVIRONMENT"), os.environ.get("ODOO_URL")
        try:
            os.environ.update(selected)
            os.environ["FURNIBOX_ENVIRONMENT"] = self.environment.get().upper()
            output_dir = environment_output_dir(BASE_DIR)
        finally:
            if old_name is None: os.environ.pop("FURNIBOX_ENVIRONMENT", None)
            else: os.environ["FURNIBOX_ENVIRONMENT"] = old_name
            if old_url is None: os.environ.pop("ODOO_URL", None)
            else: os.environ["ODOO_URL"] = old_url
        try:
