...
def build_rows(
    ...
    # APACK nėra atskiras Reform šaltinio SKU. Jį reikia tikrinti visiems
    # FPACK BOM tėvams, nepriklausomai nuo to, ar pats FPACK produktas jau
    # egzistuoja Odoo. Ankstesnė logika APACK kūrė tik tada, kai trūko ir
    # šaltinio FPACK, todėl dalis APACK kortelių Production liko nesukurtos.
    scheduled_skus = {
        canon(row["Internal reference"])
        for row in ready + review
    }

    for source_sku, product in sorted(
        reform_products.items()
    ):
        source_code = canon(source_sku)

        if not (
            product.get("is_parent")
            and source_code.startswith("FPACK-")
        ):
            continue

        generated_code = canon(
            apack_sku(source_code)
        )
        ...
        if not apack_category:
            diagnostics.append(
                f"{source_code}: nenustatyta APACK kategorija; "
        ...
        if duplicate is None:
            diagnostics.append(
                f"{source_code}: nenustatyta FPACK produkto kategorija; "
                f"kortelė {generated_code} neįtraukta"
            )
            continue
        ...
        duplicate["route_ids/id"] = (
            route_profile["apack"]
        )
        duplicate["categ_id"] = (
            apack_category["external_id"]
        )
        duplicate["packaging_name"] = (
            packaging_name(source_code)
        )
        duplicate["Reform Role"] = (
            "GENERATED APACK"
        )
        duplicate[
